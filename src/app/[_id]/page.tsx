...
const Page = async ({ searchParams }: Props) => {
  ...
  const singleProduct = (_id: number) => {
    const item = products.find((products: ProductType) => products._id === _id);
    return item;
  };
  ...
  return (
    <Container className={'flex items-center flex-col md:flex-row px-4 xl:px-0'}>
      <div
        className={
          'w-full md:w-1/2 overflow-hidden bg-zinc-50 flex items-center justify-center p-10'
        }
      >
        <Image
          src={product?.image}
      ...
      <div className={'w-full md:w-1/2 flex flex-col gap-2'}>
        ...
        <button
          className={
            'bg-designColor/80 text-zinc-700 px-6 py-2 font-medium rounded-md hover:bg-designColor hover:text-black cursor-pointer duration-200 hover:shadow-lg w-40 my-2'
          }
        >
          add to cart
        </button>
